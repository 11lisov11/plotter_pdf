        ...
        if bool(selected.get("ok")):
            ...
            rows.append(
                prep.ArtifactRow(
                    ...
                    selected_variant=str(selected.get("variant_label", "") or ""),
                    source_fidelity_score=float(selected.get("layout_similarity", 0.0) or 0.0),
                    fragmentation_score=prep._candidate_fragmentation_score(dict(q)),
                    draw_length_m=round(float(q.get("draw_length_mm", 0.0)) / 1000.0, 3),
                    segments_total=int(q.get("segments_total", 0) or 0),
                    pen_down_strokes=int(q.get("pen_down_strokes", 0) or 0),
                    tiny_strokes_lt_08_mm=int(q.get("tiny_strokes_lt_08_mm", 0) or 0),
                    point_like_strokes=int(q.get("point_like_strokes", 0) or 0),
