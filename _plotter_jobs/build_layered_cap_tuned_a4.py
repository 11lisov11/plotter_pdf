...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_layered_cap_tuned_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
# capped tonal groups
bundles('forest_light',forest,.050,31,2,[-62,58],(12,26),1,2.3,10,.84,11,245)
bundles('forest_dark',forest,.155,23,2,[-65,55,86],(10,23),2,2.3,12,.80,12,420)
bundles('forest_deep',forest,.290,21,2,[-68,52,88],(8,18),2,2.1,14,.78,13,285)
bundles('field_light',field,.050,40,1,[-14,-9],(24,60),1,2.6,7,.90,21,115)
bundles('field_mid',field,.155,34,1,[-15,16],(18,42),2,2.5,7,.92,22,70)
bundles('grass',grass,.075,27,2,[-82,76,-68],(10,29),2,2.1,12,.92,31,245)
bundles('jacket_light',jacket,.065,20,2,[-52,44],(15,34),2,2.5,8,.90,41,380)
bundles('jacket_dark',jacket,.230,19,2,[-55,43,-74],(12,30),2,3.0,9,.84,42,430)
bundles('hair_shadow',hair,.145,23,1,[-72],(18,40),2,2.3,5,.86,51,135)
# sparse long field/grass gestures: restore pencil flow without filling light areas
rng_tuned=random.Random(2718)
def tuned_flow(kind, ybase, count, slope, length_rng, amp, mask):
    for i in range(count):
        x0=rng_tuned.uniform(0.05*w,0.95*w)
        y0=(ybase+rng_tuned.uniform(-0.013,0.013))*h + slope*(x0-w*0.5)
        a=math.atan(slope)+rng_tuned.uniform(-0.038,0.038)
        ln=rng_tuned.uniform(*length_rng)
        pts=[]
        for k in range(13):
            t=k/12-.5
            bend=math.sin((t+.5)*math.pi)*rng_tuned.uniform(-amp,amp)
            pts.append((x0+math.cos(a)*ln*t-math.sin(a)*bend, y0+math.sin(a)*ln*t+math.cos(a)*bend))
        clip(kind,pts,mask,16)
tuned_flow('field_flow_soft',0.535,10,-0.12,(54,112),2.0,field)
tuned_flow('field_flow_soft',0.615,7,-0.10,(42,90),2.3,field)
tuned_flow('grass_flow_soft',0.725,8,-0.08,(38,78),2.6,grass)
def tuned_bez(p0,p1,p2,p3,n=42):
    pts=[]
    for ii in range(n):
        tt=ii/(n-1)
    ...
    return pts
# jacket dark mass: a few longer fold strokes so density reads as fabric, not just random knots
for i in range(22):
    t=i/21
    x0=(0.340+0.195*t+rng_tuned.uniform(-0.010,0.010))*w
    y0=(0.715+rng_tuned.uniform(-0.010,0.010))*h
    x3=(0.360+0.175*t+rng_tuned.uniform(-0.012,0.012))*w
    y3=(0.965+rng_tuned.uniform(-0.010,0.010))*h
    clip('jacket_long_fold',tuned_bez((x0,y0),((x0+x3)/2-0.045*w,0.790*h),((x0+x3)/2+0.025*w,0.895*h),(x3,y3),42),jacket,18)
# foreground grass gestures, reduced count and curved like hand marks
for i in range(58):
    cx=rng_tuned.uniform(0.03*w,0.97*w); cy=rng_tuned.uniform(0.710*h,0.965*h)
    ix=int(min(w-1,max(0,cx))); iy=int(min(h-1,max(0,cy)))
    if not grass[iy,ix]:
        continue
    a=math.radians(rng_tuned.choice([-82,-70,72,82])+rng_tuned.uniform(-7,7)); ln=rng_tuned.uniform(13,30)
    pts=[]
    for k in range(6):
        t=k/5-.5; bend=math.sin((t+.5)*math.pi)*rng_tuned.uniform(-2.0,2.0)
...
name='gemini_layered_cap_tuned_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; layered cap tuned A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: tuned capped density; fewer random light strokes, added sparse field flow and long jacket folds while preserving dark close hatching.\n',encoding='utf-8')
print('LAYERED CAP TUNED A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
