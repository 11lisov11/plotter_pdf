...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_organic_density_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('forest_dark',forest,.145,23,2,[-65,55,86],(10,23),2,2.1,12,.78,12,420)
bundles('forest_deep',forest,.275,20,2,[-68,52,88],(8,18),2,2.0,14,.75,13,290)
bundles('field_light',field,.040,42,1,[-14,-9],(28,72),1,2.4,7,.88,21,130)
bundles('field_mid',field,.135,34,1,[-15,16],(22,54),2,2.2,7,.90,22,105)
bundles('grass',grass,.070,26,2,[-82,76,-68],(10,30),2,1.9,12,.90,31,250)
bundles('jacket_light',jacket,.060,19,2,[-52,44],(15,34),2,2.1,8,.87,41,430)
bundles('jacket_dark',jacket,.215,18,2,[-55,43,-74],(12,30),2,2.5,9,.80,42,480)
bundles('hair_shadow',hair,.145,22,1,[-72],(18,38),2,2.1,5,.84,51,135)
# long organic field strokes following the hillside, replacing random field scratchiness
rng_flow=random.Random(909)
def flow_curve(kind, ybase, count, slope=-0.11, amp=4.0, length_rng=(70,150), xspan=(0.02,0.98), mask=field):
    for i in range(count):
        x0=rng_flow.uniform(xspan[0]*w,xspan[1]*w)
        y0=(ybase+rng_flow.uniform(-0.018,0.018))*h + slope*(x0-w*0.5)
        ln=rng_flow.uniform(*length_rng)
        ang=math.atan(slope)+rng_flow.uniform(-0.045,0.045)
        pts=[]
        for k in range(18):
            t=k/17-.5
            ...
            pts.append((x,y))
        clip(kind,pts,mask,18)
flow_curve('field_flow_light',0.535,32,slope=-0.12,amp=2.8,length_rng=(85,170),mask=field)
flow_curve('field_flow_mid',0.610,26,slope=-0.10,amp=3.2,length_rng=(70,145),mask=field)
flow_curve('field_flow_low',0.705,24,slope=-0.08,amp=3.8,length_rng=(55,120),mask=grass)
# a few curved foreground grass gestures, grouped instead of random vertical rain
for i in range(95):
    cx=rng_flow.uniform(0.02*w,0.98*w); cy=rng_flow.uniform(0.70*h,0.96*h)
    if not grass[int(min(h-1,max(0,cy))), int(min(w-1,max(0,cx)) )]:
        continue
    a=math.radians(rng_flow.choice([-82,-70,72,82])+rng_flow.uniform(-8,8)); ln=rng_flow.uniform(14,34)
    pts=[]
    for k in range(6):
        t=k/5-.5; bend=math.sin((t+.5)*math.pi)*rng_flow.uniform(-2.5,2.5)
...
name='gemini_organic_density_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; organic density A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: organic capped density; dark areas use close parallel groups, light areas use sparse long gestures, field/grass are organized by flow lines.\n',encoding='utf-8')
print('ORGANIC DENSITY A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
