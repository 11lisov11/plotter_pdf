...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_tonal_threshold_layers_a4_pack")
...
broken_hatch("forest_light", forest, .055, -62, 18, 5, 24, (20, 48), .72, .85, phase=2, close=6, seed=20)
broken_hatch("forest_mid", forest, .135, 56, 14, 5, 20, (16, 38), .78, .75, phase=6, close=5, seed=21)
broken_hatch("forest_dark", forest, .235, 86, 10, 5, 15, (12, 30), .68, .55, phase=1, close=4, seed=22)
broken_hatch("forest_deep", forest, .315, -70, 8.5, 4, 13, (10, 24), .48, .45, phase=4, close=3, seed=23)

broken_hatch("field_light", field, .055, -13, 17, 6, 42, (45, 105), .60, .60, phase=4, close=8, seed=30)
broken_hatch("field_mid", field, .150, 16, 23, 6, 32, (30, 70), .38, .50, phase=7, close=6, seed=31)

broken_hatch("grass_light", grass, .080, -80, 17, 5, 14, (12, 32), .42, .65, phase=2, close=4, seed=40)
broken_hatch("grass_mid", grass, .170, 75, 16, 5, 13, (10, 28), .32, .55, phase=5, close=4, seed=41)

broken_hatch("jacket_light", jacket, .065, -52, 9.5, 4, 18, (22, 48), .78, .45, phase=0, close=5, seed=50)
broken_hatch("jacket_mid", jacket, .145, 44, 9.2, 4, 17, (20, 44), .76, .42, phase=3, close=5, seed=51)
broken_hatch("jacket_dark", jacket, .255, -74, 8.5, 4, 15, (16, 36), .54, .34, phase=1, close=4, seed=52)

broken_hatch("hair_shadow", hair, .155, -72, 16, 4, 20, (20, 46), .35, .35, phase=2, close=5, seed=60)
...
name = "gemini_tonal_threshold_layers_a4"
nc = OUT / f"{name}.nc"
gcode = OUT / f"{name}.gcode"
lines = ["; tonal threshold layers A4", "G21", "G90", f"G0 Z{UP:.3f}", f"F{TF}"]
draw = travel = 0.0
last = (0.0, 0.0)
...
(OUT / "README_result.txt").write_text(
    "TONAL THRESHOLD LAYERS A4 package\n" +
    f"source: {SRC}\n" + f"nc: {nc}\n" + f"gcode: {gcode}\n" +
    ...
    "algorithm_note: broken tonal threshold hatch layers; light sparse, dark close multi-layer chunks, with semantic hair/fabric strokes.\n",
    encoding="utf-8")
print("TONAL THRESHOLD LAYERS A4 package")
