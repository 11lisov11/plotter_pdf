...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_pencil_density_tilebundles_a4_pack")
...
def tile_bundles(kind: str, mask: np.ndarray, threshold: float, tile_px: int, max_strokes: int,
                 angles_deg: list[float], length_rng: tuple[float, float], jitter_deg: float,
                 density_gamma: float = 1.0, seed: int = 0) -> None:
    rng = random.Random(9000 + seed)
    y0s = list(range(0, h, tile_px))
    x0s = list(range(0, w, tile_px))
    for y0 in y0s:
        for x0 in x0s:
            y1 = min(h, y0 + tile_px)
            x1 = min(w, x0 + tile_px)
            local_mask = mask[y0:y1, x0:x1]
            if np.count_nonzero(local_mask) < max(8, tile_px * tile_px * 0.12):
                continue
            vals = tone[y0:y1, x0:x1][local_mask]
            avg = float(np.mean(vals)) if vals.size else 0.0
            if avg < threshold:
                continue
            strength = min(1.0, max(0.0, (avg - threshold) / max(1e-6, 1.0 - threshold))) ** density_gamma
            want = strength * max_strokes
            n = int(want)
            if rng.random() < (want - n):
                n += 1
            if n <= 0:
                n = 1
            # Dark cells get an extra cross direction, but still as grouped strokes.
            if avg > threshold + 0.18 and max_strokes >= 3 and rng.random() < 0.55:
                n += 1
            ...
            for j in range(n):
                ...
                cx = x0 + float(xs[k]) + rng.uniform(-0.16, 0.16) * tile_px
                cy = y0 + float(ys[k]) + rng.uniform(-0.16, 0.16) * tile_px
                if not (0 <= int(cx) < w and 0 <= int(cy) < h and mask[int(cy), int(cx)]):
                    continue
                angle = math.radians(rng.choice(angles_deg) + rng.uniform(-jitter_deg, jitter_deg))
                ln = rng.uniform(*length_rng) * (0.72 + 0.85 * avg)
                dx = math.cos(angle) * ln * 0.5
                dy = math.sin(angle) * ln * 0.5
                pts = []
                for q in range(5):
                    t = q / 4 - 0.5
                    wob = math.sin((q / 4) * math.pi) * rng.uniform(-1.1, 1.1)
                    nx = -math.sin(angle); ny = math.cos(angle)
                    x = cx + 2 * t * dx + nx * wob
                    y = cy + 2 * t * dy + ny * wob
                    pts.append((x, y))
                clip_add(kind, pts, mask, min_len_px=max(7.0, length_rng[0] * 0.45))

# Forest/field/grass are built as tonal bundles, not long ruler-lines and not pixel trash.
tile_bundles("forest_bundle_light", forest_mask, 0.055, tile_px=24, max_strokes=2, angles_deg=[-62, 58], length_rng=(11, 24), jitter_deg=11, density_gamma=0.80, seed=1)
tile_bundles("forest_bundle_dark", forest_mask, 0.150, tile_px=18, max_strokes=4, angles_deg=[-65, 55, 86], length_rng=(9, 22), jitter_deg=13, density_gamma=0.75, seed=2)
tile_bundles("field_bundle_light", field_mask, 0.030, tile_px=30, max_strokes=2, angles_deg=[-13], length_rng=(22, 58), jitter_deg=8, density_gamma=0.80, seed=3)
tile_bundles("field_bundle_mid", field_mask, 0.110, tile_px=26, max_strokes=2, angles_deg=[-13, 17], length_rng=(18, 46), jitter_deg=8, density_gamma=0.85, seed=4)
tile_bundles("grass_bundle", grass_mask, 0.055, tile_px=20, max_strokes=3, angles_deg=[-82, 76, -68], length_rng=(10, 28), jitter_deg=12, density_gamma=0.85, seed=5)
...
name = "gemini_pencil_density_tilebundles_a4"
nc_path = OUT / f"{name}.nc"
gcode_path = OUT / f"{name}.gcode"
lines = ["; pencil density tilebundles A4", "G21", "G90", f"G0 Z{PEN_UP_Z:.3f}", f"F{TRAVEL_F}"]
...
readme.write_text(
    "PENCIL DENSITY TILEBUNDLES A4 package\n"
    f"source: {SRC}\n"
    f"nc: {nc_path}\n"
    f"gcode: {gcode_path}\n"
...
print("PENCIL DENSITY TILEBUNDLES A4 package")
