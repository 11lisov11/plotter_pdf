...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_plus_grassdetail_a4_pack")
...
def add_grass_blades(paths, dens, reg):
    h, w = dens.shape
    yy, xx = np.mgrid[0:h, 0:w]
    grass_zone = reg['grass'] & (yy > h*0.64) & (dens > 0.075)
    # Blue-noise-ish grid: enough grass texture, but not a dust cloud.
    step = 22
    for gy in np.arange(h*0.64 + RNG.uniform(0, step), h-8, step):
        for gx in np.arange(RNG.uniform(0, step), w, step):
            x = gx + RNG.uniform(-7, 7)
            y = gy + RNG.uniform(-7, 7)
            ix = int(np.clip(round(x), 0, w-1)); iy = int(np.clip(round(y), 0, h-1))
            d = float(dens[iy, ix])
            if not grass_zone[iy, ix]:
                continue
            # Sparse in light grass, extra blades only in darker patches.
            reps = 1
            if d > 0.23: reps = 2
            if d > 0.40: reps = 3
            for r in range(reps):
                ang = math.radians(RNG.choice([-76, -62, -48, 58, 70]) + RNG.uniform(-9, 9))
                length = RNG.uniform(9, 23) * (1.0 + min(d, 0.55)*0.55)
                x0 = x + RNG.uniform(-4, 4)
                y0 = y + RNG.uniform(-4, 4)
                pts=[]
                phase = RNG.random()*math.tau
                for t in np.linspace(0, 1, 9):
                    bend = math.sin(t*math.pi + phase)*1.6*t
                    px = x0 + math.cos(ang)*length*t + math.cos(ang+math.pi/2)*bend
                    py = y0 + math.sin(ang)*length*t + math.sin(ang+math.pi/2)*bend
                    jx=int(np.clip(round(px),0,w-1)); jy=int(np.clip(round(py),0,h-1))
                    if not reg['grass'][jy, jx]:
                        break
                    pts.append((float(px), float(py)))
                if len(pts) >= 3:
                    paths.append({'pts_px': pts, 'strength': max(0.14, d), 'kind': 'grass_blade_detail'})
...
def write_gcode(paths, nc):
    lines=['; density plus grass detail A4','G21','G90',f'G0 Z{PEN_UP_Z:.3f}',f'F{TRAVEL_F}']
...
def main():
    ...
    nc=OUT/'gemini_density_plus_grassdetail_a4.nc'; gcode=OUT/'gemini_density_plus_grassdetail_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_plus_grassdetail_preview_pressure_gray.png'; bpng=OUT/'gemini_density_plus_grassdetail_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_plus_grassdetail_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_plus_grassdetail_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY PLUS GRASS DETAIL A4 package\n'
        ...
        'algorithm_note: plus_cloudforest with added controlled grass-blade detail in the foreground; figure and sky density unchanged. Light areas receive sparse long hatches; each darker threshold adds closer crossing layers. Hair uses curved strands, jacket uses clean close crosshatch, and only long contours are kept.\n'
        'files:\n- gemini_density_plus_grassdetail_preview_pressure_gray.png/pdf\n- gemini_density_plus_grassdetail_preview_black_actual.png/pdf\n- gemini_density_plus_grassdetail_a4.nc\n- gemini_density_plus_grassdetail_a4.gcode\n'
    )
