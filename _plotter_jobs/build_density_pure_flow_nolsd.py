...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_pure_flow_nolsd_a4_pack")
...
def main():
    ...
    # No LSD detail layer here: it caused fragmented black-pen noise.
    add_contours(paths, gray, dens)
    ordered, draw_w, draw_h = convert_order_write(paths, gray.shape[1], gray.shape[0])
    nc=OUT/'gemini_density_pure_flow_nolsd_a4.nc'; gcode=OUT/'gemini_density_pure_flow_nolsd_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_pure_flow_nolsd_preview_pressure_gray.png'
    bpng=OUT/'gemini_density_pure_flow_nolsd_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_pure_flow_nolsd_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_pure_flow_nolsd_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY PURE FLOW NO-LSD A4 package\n'
        f'source: {SRC}\noutput_dir: {OUT}\n'
        ...
        'algorithm_note: region-aware hatch layers without LSD fragment noise. Light zones receive sparse long strokes; dark zones receive multiple closer hatch directions; only long source contours are kept for readability.\n'
        'files:\n- gemini_density_pure_flow_nolsd_preview_pressure_gray.png/pdf\n- gemini_density_pure_flow_nolsd_preview_black_actual.png/pdf\n- gemini_density_pure_flow_nolsd_a4.nc\n- gemini_density_pure_flow_nolsd_a4.gcode\n'
    )
