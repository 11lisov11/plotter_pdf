...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_structured_grattage_max_a4_pack")
...
tone = gaussian_filter(tone_raw, sigma=2.2)
tone = np.clip((tone - 0.025) / 0.42, 0, 1)
...
binary = tone_strong > 0.145
binary = binary_closing(binary, structure=np.ones((2, 2), dtype=bool))
binary = remove_small_objects(binary, min_size=26)
...
for cnt in contours:
    ...
    if local_dark < 0.085 and arc < 55:
        continue
...
hatch_layer("sky_soft", sky_mask, 0.115, -25, spacing=24, step=6, min_run=45, jitter=1.1, keep=0.72, phase=3)
hatch_layer("sky_cross", sky_mask, 0.185, 28, spacing=30, step=6, min_run=38, jitter=0.8, keep=0.55, phase=9)
...
hatch_layer("field_slope", field_mask, 0.080, -13, spacing=15, step=6, min_run=55, jitter=0.9, keep=0.78, phase=4)
hatch_layer("field_cross", field_mask, 0.180, 17, spacing=18, step=6, min_run=42, jitter=0.8, keep=0.55, phase=7)
# Grass lower zone: controlled clumps, only where tone supports it.
hatch_layer("grass_base", grass_mask, 0.105, -82, spacing=15, step=5, min_run=18, jitter=1.0, keep=0.72, phase=2)
hatch_layer("grass_cross", grass_mask, 0.185, 76, spacing=17, step=5, min_run=16, jitter=0.9, keep=0.55, phase=6)
...
# Save G-code / NC.
name = "gemini_structured_grattage_max_a4"
nc_path = OUT / f"{name}.nc"
gcode_path = OUT / f"{name}.gcode"
lines = [
    "; structured grattage max A4 - coherent density hatching",
    "G21",
    "G90",
    f"G0 Z{PEN_UP_Z:.3f}",
    f"F{TRAVEL_F}",
...
readme.write_text(
    "STRUCTURED GRATTAGE MAX A4 package\n"
    f"source: {SRC}\n"
    f"nc: {nc_path}\n"
    f"gcode: {gcode_path}\n"
...
print("STRUCTURED GRATTAGE MAX A4 package")
