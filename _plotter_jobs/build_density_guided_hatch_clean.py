...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_guided_clean_a4_pack")
...
def write_gcode(paths: list[dict], nc: Path):
    lines=['; density guided clean hatch A4', 'G21', 'G90', f'G0 Z{PEN_UP_Z:.3f}', f'F{TRAVEL_F}']
...
def main():
    ...
    add_hatches(paths,dens,regions['jacket'],angle_deg=-54,spacing=6,threshold=0.12,min_len=16,max_len=85,jitter=0.35,label='jacket_a')
    add_hatches(paths,dens,regions['jacket'],angle_deg=52,spacing=6,threshold=0.18,min_len=16,max_len=85,jitter=0.35,label='jacket_b')
    add_hatches(paths,dens,regions['jacket'],angle_deg=12,spacing=7,threshold=0.34,min_len=14,max_len=75,jitter=0.28,label='jacket_deep')
    add_hatches(paths,dens,regions['body'],angle_deg=-38,spacing=9,threshold=0.20,min_len=15,max_len=75,jitter=0.32,label='body_mid')
    ...
    nc=OUT/'gemini_density_guided_clean_a4.nc'; gcode=OUT/'gemini_density_guided_clean_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_guided_clean_preview_pressure_gray.png'
    bpng=OUT/'gemini_density_guided_clean_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_guided_clean_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_guided_clean_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY GUIDED CLEAN HATCH A4 package\n'
        f'source: {SRC}\noutput_dir: {OUT}\n'
        ...
        'algorithm_note: region-aware hatch layers. Light zones receive sparse long strokes; dark zones receive multiple closer hatch directions; source LSD/contours are filtered as detail only; jacket/body use longer clean crosshatch to reduce black-pen grain.\n'
        'files:\n- gemini_density_guided_clean_preview_pressure_gray.png/pdf\n- gemini_density_guided_clean_preview_black_actual.png/pdf\n- gemini_density_guided_clean_a4.nc\n- gemini_density_guided_clean_a4.gcode\n'
    )
