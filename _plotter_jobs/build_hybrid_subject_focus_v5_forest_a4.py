...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_hybrid_subject_focus_v5_forest_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('forest_dark',forest,.145,23,2,[-65,55,-58,48],(10,23),2,2.2,11,.80,12,430)
bundles('forest_deep',forest,.285,21,2,[-68,52,-56],(8,18),2,2.1,12,.78,13,300)
bundles('forest_texture_diag',forest,.105,24,1,[-58,48],(10,24),1,2.0,9,.86,14,120)
...
bundles('jacket_light',jacket,.064,20,2,[-52,44],(15,34),2,2.4,8,.90,41,430)
bundles('jacket_dark',jacket,.225,19,2,[-55,43,-74],(12,30),2,2.9,9,.84,42,500)
...
name='gemini_hybrid_subject_focus_v5_forest_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; hybrid subject focus v5 forest A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: hybrid subject focus v3; clustered grass near subject plus sparse right-bottom patch, preserving dark subject/forest density and light open field.\n',encoding='utf-8')
print('HYBRID SUBJECT FOCUS V5 FOREST A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
