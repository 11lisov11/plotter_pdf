...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_fullfield_plus_skyfield_a4_pack")
...
def write_gcode(paths, nc):
    lines=['; density fullfield plus sky field A4','G21','G90',f'G0 Z{PEN_UP_Z:.3f}',f'F{TRAVEL_F}']
...
def main():
    ...
    add_layer(paths,dens,reg['sky'],angle=28,spacing=32,threshold=0.17,min_len=20,max_len=82,label='sky_light',jitter=0.22,bridge_px=3)
    add_layer(paths,dens,reg['sky'],angle=-24,spacing=44,threshold=0.30,min_len=18,max_len=68,label='sky_dark_cross',jitter=0.20,bridge_px=2)
    # Forest: density staircase, each darker level adds another direction.
    add_layer(paths,dens,reg['forest'],angle=-70,spacing=12,threshold=0.10,min_len=8,max_len=54,label='forest_l1',jitter=0.38,bridge_px=5)
    add_layer(paths,dens,reg['forest'],angle=-40,spacing=11,threshold=0.20,min_len=8,max_len=50,label='forest_l2',jitter=0.34,bridge_px=4)
    ...
    add_layer(paths,dens,reg['field'],angle=12,spacing=13,threshold=0.07,min_len=28,max_len=170,label='field_l1',jitter=0.22,bridge_px=7)
    add_layer(paths,dens,reg['field'],angle=20,spacing=12,threshold=0.18,min_len=18,max_len=130,label='field_l2',jitter=0.20,bridge_px=5)
    ...
    nc=OUT/'gemini_density_fullfield_plus_skyfield_a4.nc'; gcode=OUT/'gemini_density_fullfield_plus_skyfield_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_fullfield_plus_skyfield_preview_pressure_gray.png'; bpng=OUT/'gemini_density_fullfield_plus_skyfield_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_fullfield_plus_skyfield_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_fullfield_plus_skyfield_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY FULLFIELD PLUS SKY/FIELD A4 package\n'
        ...
        'algorithm_note: tonal staircase with extra sparse sky/cloud and field texture, without dark-area bundling. Light areas receive sparse long hatches; each darker threshold adds closer crossing layers. Hair uses curved strands, jacket uses clean close crosshatch, and only long contours are kept.\n'
        'files:\n- gemini_density_fullfield_plus_skyfield_preview_pressure_gray.png/pdf\n- gemini_density_fullfield_plus_skyfield_preview_black_actual.png/pdf\n- gemini_density_fullfield_plus_skyfield_a4.nc\n- gemini_density_fullfield_plus_skyfield_a4.gcode\n'
    )
