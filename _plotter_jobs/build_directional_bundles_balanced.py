...
OUT=Path(r"C:\plotter_pdf\_plotter_jobs\gemini_directional_bundles_balanced_a4_pack")
...
def add_bundle(name,region,y0f,y1f,x0f,x1f,step,thr,prob,len_min,len_max,bias,seed,min_active=.52):
    ...
    for gy,y0 in enumerate(range(y0i,y1i,sy)):
        for gx,x0 in enumerate(range(x0i,x1i,sx)):
            ...
            companions=1 + (1 if d>.70 and h01(gx,gy,seed+6)<.16 else 0)
...
counts['forest_mid_bundles']=add_bundle('forest_mid_bundles','forest',.34,.62,.0,1.0,11.5,.21,.46,10,28,-.04,20,.52)
counts['forest_dark_bundles']=add_bundle('forest_dark_bundles','forest',.34,.62,.0,1.0,8.5,.43,.46,10,30,.10,30,.50)
counts['field_flow_bundles']=add_bundle('field_flow_bundles','field',.50,.78,.0,1.0,10.5,.18,.42,28,76,-.09,40,.60)
counts['field_dark_bundles']=add_bundle('field_dark_bundles','field',.50,.78,.0,1.0,9.5,.43,.34,18,52,.03,45,.56)
counts['grass_light_bundles']=add_bundle('grass_light_bundles','grass',.68,.99,.0,1.0,8.8,.18,.42,8,22,-.06,50,.48)
counts['grass_dark_bundles']=add_bundle('grass_dark_bundles','grass',.68,.99,.0,1.0,7.2,.45,.44,8,24,.10,60,.48)
counts['figure_mid_bundles']=add_bundle('figure_mid_bundles','figure',.60,.99,.24,.66,7.2,.27,.54,10,30,.06,70,.50)
counts['figure_dark_bundles']=add_bundle('figure_dark_bundles','figure',.60,.99,.24,.66,6.2,.50,.54,12,34,.18,80,.48)
counts['hair_flow_bundles']=add_bundle('hair_flow_bundles','hair',.52,.80,.38,.76,6.5,.20,.58,15,46,-.03,90,.50)
# Border.
...
render(OUT/'gemini_directional_bundles_balanced_preview_pressure_gray.png',True)
render(OUT/'gemini_directional_bundles_balanced_preview_black_actual.png',False,True)
render(OUT/'gemini_directional_bundles_balanced_preview_dark_pressure.png',True,False,True)
SAFE=13.0
lines=['; gemini_directional_bundles_balanced_a4','; balanced real trace plus directional tone bundles: dark close lines, light sparse, no G0 drawing','G21','G90',f'G0 Z{SAFE:.2f}','G0 X0.000 Y0.000']
...
(OUT/'gemini_directional_bundles_balanced_a4.nc').write_text('\n'.join(lines)+'\n',encoding='utf-8')
(OUT/'gemini_directional_bundles_balanced_a4.gcode').write_text('\n'.join(lines)+'\n',encoding='utf-8')
readme=f"""DIRECTIONAL BUNDLES BALANCED A4 package
source: {SRC}
output_dir: {OUT}
...
algorithm_note: preserves real extracted source strokes, then adds reduced region-aware directional bundles. Dark zones get close strokes without black mass; light sky/cloud regions are sparse and threshold-protected.
files:
- gemini_directional_bundles_balanced_preview_pressure_gray.png/pdf
- gemini_directional_bundles_balanced_preview_black_actual.png/pdf
- gemini_directional_bundles_balanced_preview_dark_pressure.png/pdf
- gemini_directional_bundles_balanced_a4.nc
- gemini_directional_bundles_balanced_a4.gcode
"""
