...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_hybrid_subject_focus_v4_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('field_light',field,.060,42,1,[-14,-9],(24,58),1,2.7,7,.93,21,92)
bundles('field_mid',field,.165,36,1,[-15,16],(18,42),2,2.6,7,.94,22,58)
bundles('grass',grass,.110,30,2,[-82,76,-68],(10,28),2,2.2,12,.96,31,82)
bundles('jacket_light',jacket,.070,22,2,[-52,44],(15,34),2,2.8,8,.92,41,310)
bundles('jacket_dark',jacket,.245,21,2,[-55,43,-74],(12,30),2,3.2,9,.86,42,340)
bundles('hair_shadow',hair,.145,22,1,[-72],(18,38),2,2.2,5,.84,51,150)
# jacket fabric strokes: coherent folds instead of only random dark knots
rng_jacket=random.Random(8844)
def local_bez(p0,p1,p2,p3,n=44):
    pts=[]
    for ii in range(n):
        tt=ii/(n-1)
        pts.append(((1-tt)**3*p0[0]+3*(1-tt)**2*tt*p1[0]+3*(1-tt)*tt*tt*p2[0]+tt**3*p3[0],
                    (1-tt)**3*p0[1]+3*(1-tt)**2*tt*p1[1]+3*(1-tt)*tt*tt*p2[1]+tt**3*p3[1]))
    return pts
# long vertical/curved folds on the body mass
for i in range(34):
    t=i/33
    x0=(0.335+0.235*t+rng_jacket.uniform(-0.010,0.010))*w
    y0=(0.690+0.030*math.sin(t*math.pi)+rng_jacket.uniform(-0.006,0.006))*h
    x3=(0.350+0.205*t+rng_jacket.uniform(-0.012,0.012))*w
    y3=(0.968+rng_jacket.uniform(-0.008,0.008))*h
    clip('jacket_fabric_fold',local_bez((x0,y0),((x0+x3)/2-0.048*w,0.780*h),((x0+x3)/2+0.020*w,0.900*h),(x3,y3),46),jacket,18)
# short sleeve/back hatch groups, dark but structured
sleeve_zone = jacket & ((((xn-0.360)/0.130)**2 + ((yn-0.770)/0.135)**2) < 1.0)
bundles('jacket_sleeve_hatch',sleeve_zone,.085,18,2,[-48,42],(12,28),2,2.3,8,.84,81,145)
body_shadow_zone = jacket & ((((xn-0.475)/0.155)**2 + ((yn-0.865)/0.160)**2) < 1.0)
bundles('jacket_body_shadow_hatch',body_shadow_zone,.095,18,2,[-58,36],(13,30),2,2.4,8,.84,82,175)
...
hybrid_flow('field_flow_soft',0.535,10,-0.12,(54,115),2.2,field)
hybrid_flow('field_flow_soft',0.615,7,-0.10,(42,88),2.4,field)
hybrid_flow('grass_flow_soft',0.725,8,-0.08,(36,76),2.7,grass)
# subject-focus dark accents near the figure: keep density where it belongs, not evenly everywhere
subject_shadow = grass & (((xn-0.470)/0.250)**2 + ((yn-0.820)/0.210)**2 < 1.0)
bundles('subject_ground_shadow',subject_shadow,.068,22,2,[-78,72,-55],(10,28),2,1.8,11,.84,71,118)
# clustered foreground grass: not uniform rain, only tone/subject-weighted clumps
foreground_clumps = grass & (yn > 0.710) & ((tone > 0.070) | ((((xn-0.455)/0.330)**2 + ((yn-0.845)/0.220)**2) < 1.0))
bundles('grass_clump_foreground',foreground_clumps,.055,20,2,[-82,74,-66],(10,31),2,1.7,11,.82,72,155)
# a softer sparse patch on the left-bottom like the reference grass, not across the whole page
left_grass_patch = grass & (yn > 0.700) & (xn < 0.390) & (tone > 0.040)
bundles('grass_clump_left',left_grass_patch,.042,24,1,[-82,-70],(11,34),1,1.9,10,.86,73,75)
# sparse right-bottom patch: fills the empty corner without returning to uniform rain
right_grass_patch = grass & (yn > 0.700) & (xn > 0.610) & ((tone > 0.038) | ((((xn-0.760)/0.250)**2 + ((yn-0.830)/0.200)**2) < 1.0))
bundles('grass_clump_right',right_grass_patch,.040,28,1,[-82,-70,74],(10,27),1,1.9,10,.88,74,58)
...
name='gemini_hybrid_subject_focus_v4_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; hybrid subject focus v4 A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: hybrid subject focus v4; v3 balance plus more coherent fabric folds in the jacket, reducing random dark knots.\n',encoding='utf-8')
print('HYBRID SUBJECT FOCUS V4 A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
