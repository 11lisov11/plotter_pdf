...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_directed_density_refined_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
# capped tonal groups
bundles('forest_light',forest,.045,30,2,[-62,58],(12,26),1,2.2,10,.82,11,250)
bundles('forest_dark',forest,.150,23,2,[-65,55,86],(10,23),2,2.2,12,.80,12,430)
bundles('forest_deep',forest,.285,21,2,[-68,52,88],(8,18),2,2.1,14,.78,13,300)
bundles('field_light',field,.042,38,1,[-14,-9],(25,62),1,2.5,7,.90,21,135)
bundles('field_mid',field,.145,32,1,[-15,16],(18,44),2,2.4,7,.92,22,80)
bundles('grass',grass,.075,25,2,[-82,76,-68],(10,28),2,2.1,12,.92,31,230)
bundles('jacket_light',jacket,.060,19,2,[-52,44],(15,34),2,2.2,8,.88,41,470)
bundles('jacket_dark',jacket,.220,18,2,[-55,43,-74],(12,30),2,2.6,9,.82,42,520)
bundles('hair_shadow',hair,.145,22,1,[-72],(18,38),2,2.2,5,.84,51,135)
# organic low-density gestures: give field/grass direction without turning light zones into technical stripes
rng_flow=random.Random(1909)
def flow_curve(kind, ybase, count, slope=-0.11, amp=2.8, length_rng=(55,125), mask=field):
    for i in range(count):
        x0=rng_flow.uniform(0.05*w,0.95*w)
        y0=(ybase+rng_flow.uniform(-0.014,0.014))*h + slope*(x0-w*0.5)
        ln=rng_flow.uniform(*length_rng)
        ang=math.atan(slope)+rng_flow.uniform(-0.040,0.040)
        pts=[]
        for k in range(14):
            t=k/13-.5
            x=x0+math.cos(ang)*ln*t
            y=y0+math.sin(ang)*ln*t+math.sin((t+.5)*math.pi)*rng_flow.uniform(-amp,amp)
            pts.append((x,y))
        clip(kind,pts,mask,16)
flow_curve('field_flow_light',0.535,12,slope=-0.12,amp=2.3,length_rng=(55,120),mask=field)
flow_curve('field_flow_mid',0.615,8,slope=-0.10,amp=2.6,length_rng=(45,95),mask=field)
flow_curve('field_flow_low',0.720,8,slope=-0.08,amp=3.0,length_rng=(38,84),mask=grass)
# foreground grass: fewer strokes, but curved and grouped like pencil gestures
for i in range(82):
    cx=rng_flow.uniform(0.03*w,0.97*w); cy=rng_flow.uniform(0.700*h,0.965*h)
    ix=int(min(w-1,max(0,cx))); iy=int(min(h-1,max(0,cy)))
    if not grass[iy,ix]:
        continue
    a=math.radians(rng_flow.choice([-82,-70,72,82])+rng_flow.uniform(-7,7)); ln=rng_flow.uniform(13,32)
    pts=[]
    for k in range(6):
        t=k/5-.5; bend=math.sin((t+.5)*math.pi)*rng_flow.uniform(-2.2,2.2)
        pts.append((cx+math.cos(a)*ln*t-math.sin(a)*bend, cy+math.sin(a)*ln*t+math.cos(a)*bend))
    clip('grass_gesture',pts,grass,7)
# a handful of recognizable evergreen accents in the right forest; sparse, not filled
for i in range(42):
    cx=rng_flow.uniform(0.64*w,0.97*w); cy=rng_flow.uniform(0.350*h,0.500*h)
    ix=int(min(w-1,max(0,cx))); iy=int(min(h-1,max(0,cy)))
    if not forest[iy,ix]:
        continue
    ht=rng_flow.uniform(18,42); base=cy+ht*0.45; top=cy-ht*0.55
    clip('tree_trunk',[(cx,top),(cx,base)],forest,8)
    for bidx in range(3):
        yyb=top+ht*(0.28+0.18*bidx)
        half=ht*(0.16+0.08*bidx)
        clip('tree_branch',[(cx-half,yyb+rng_flow.uniform(-1,1)),(cx,yyb-2),(cx+half,yyb+rng_flow.uniform(-1,1))],forest,7)
# sky clusters
...
name='gemini_directed_density_refined_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; directed density refined A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: directed refined density; tone bundles capped, field/grass use sparse flow gestures, dark areas remain close without solid fill.\n',encoding='utf-8')
print('DIRECTED DENSITY REFINED A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
