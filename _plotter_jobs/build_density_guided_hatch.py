...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_guided_hatch_a4_pack")
...
def add_lsd_details(paths: list[dict], gray: np.ndarray, dens: np.ndarray, regions: dict):
    ...
    for img, tag in ((norm, 'detail_norm'), (contrast, 'detail_dark')):
        ...
        for x1,y1,x2,y2 in lines.reshape(-1,4):
            ...
            if my < h*0.42 and (length < 22 or d < 0.18):
                continue
            keep = False
            if regions['hair'][my,mx] or regions['jacket'][my,mx] or regions['forest'][my,mx]:
                keep = d > 0.08 or length > 22
            elif regions['field'][my,mx] or regions['grass'][my,mx]:
                keep = d > 0.13 and length > 16
            else:
                keep = d > 0.22 and length > 18
            if keep:
...
def write_gcode(paths: list[dict], nc: Path):
    lines=['; density guided coherent hatch A4', 'G21', 'G90', f'G0 Z{PEN_UP_Z:.3f}', f'F{TRAVEL_F}']
...
def main():
    ...
    add_hatches(paths,dens,regions['sky'],angle_deg=28,spacing=34,threshold=0.24,min_len=16,max_len=70,jitter=0.35,label='sky_sparse')
    add_hatches(paths,dens,regions['sky'],angle_deg=-24,spacing=42,threshold=0.32,min_len=14,max_len=55,jitter=0.35,label='sky_cross_dark')
    ...
    add_hatches(paths,dens,regions['jacket'],angle_deg=-54,spacing=7,threshold=0.12,min_len=8,max_len=45,jitter=0.45,label='jacket_a')
    add_hatches(paths,dens,regions['jacket'],angle_deg=52,spacing=7,threshold=0.18,min_len=8,max_len=45,jitter=0.45,label='jacket_b')
    add_hatches(paths,dens,regions['jacket'],angle_deg=12,spacing=8,threshold=0.34,min_len=7,max_len=45,jitter=0.35,label='jacket_deep')
    add_hatches(paths,dens,regions['body'],angle_deg=-38,spacing=10,threshold=0.18,min_len=7,max_len=45,jitter=0.40,label='body_mid')
    # Real detected source strokes are used as detail/outline, but filtered so they don't become paper noise.
    add_lsd_details(paths, gray, dens, regions)
    add_contours(paths, gray, dens)
    ordered, draw_w, draw_h = convert_order_write(paths, gray.shape[1], gray.shape[0])
    nc=OUT/'gemini_density_guided_hatch_a4.nc'; gcode=OUT/'gemini_density_guided_hatch_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_guided_hatch_preview_pressure_gray.png'
    bpng=OUT/'gemini_density_guided_hatch_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_guided_hatch_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_guided_hatch_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY GUIDED COHERENT HATCH A4 package\n'
        f'source: {SRC}\noutput_dir: {OUT}\n'
        ...
        'algorithm_note: region-aware hatch layers. Light zones receive sparse long strokes; dark zones receive multiple closer hatch directions; source LSD/contours are filtered as detail only.\n'
        'files:\n- gemini_density_guided_hatch_preview_pressure_gray.png/pdf\n- gemini_density_guided_hatch_preview_black_actual.png/pdf\n- gemini_density_guided_hatch_a4.nc\n- gemini_density_guided_hatch_a4.gcode\n'
    )
