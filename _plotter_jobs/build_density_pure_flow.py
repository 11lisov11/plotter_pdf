...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_pure_flow_a4_pack")
...
def main():
    ...
    nc=OUT/'gemini_density_pure_flow_a4.nc'; gcode=OUT/'gemini_density_pure_flow_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_pure_flow_preview_pressure_gray.png'
    bpng=OUT/'gemini_density_pure_flow_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_pure_flow_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_pure_flow_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY PURE FLOW HATCH A4 package\n'
        f'source: {SRC}\noutput_dir: {OUT}\n'
        ...
        'files:\n- gemini_density_pure_flow_preview_pressure_gray.png/pdf\n- gemini_density_pure_flow_preview_black_actual.png/pdf\n- gemini_density_pure_flow_a4.nc\n- gemini_density_pure_flow_a4.gcode\n'
    )
