...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_hybrid_subject_focus_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('grass',grass,.090,28,2,[-82,76,-68],(10,28),2,2.1,12,.94,31,165)
...
bundles('subject_ground_shadow',subject_shadow,.070,22,2,[-78,72,-55],(10,27),2,1.8,11,.86,71,110)
...
name='gemini_hybrid_subject_focus_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; hybrid subject focus A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: hybrid subject focus; keeps dark subject/forest density while making light field and grass more sparse and less uniform.\n',encoding='utf-8')
print('HYBRID SUBJECT FOCUS A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
