...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_layered_cap_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('forest_deep',forest,.275,19,2,[-68,52,88],(8,18),2,2.0,14,.75,13,360)
bundles('field_light',field,.035,35,2,[-14,-9],(24,62),1,2.4,8,.85,21,260)
bundles('field_mid',field,.130,30,2,[-15,16],(18,45),2,2.2,8,.88,22,180)
bundles('grass',grass,.060,23,2,[-82,76,-68],(10,28),2,1.9,13,.88,31,440)
bundles('jacket_light',jacket,.060,18,2,[-52,44],(15,34),2,2.0,8,.85,41,520)
bundles('jacket_dark',jacket,.205,17,2,[-55,43,-74],(12,30),2,2.3,9,.78,42,650)
bundles('hair_shadow',hair,.135,20,2,[-72],(16,34),2,2.0,6,.82,51,240)
# sky clusters
...
name='gemini_layered_cap_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; layered cap A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: capped density tone bundles; dark areas are close but limited to preserve visible paper gaps.\n',encoding='utf-8')
print('LAYERED CAP A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
