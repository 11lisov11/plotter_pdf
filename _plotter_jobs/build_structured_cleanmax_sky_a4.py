...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_structured_cleanmax_sky_a4_pack")
...
tone = gaussian_filter(tone_raw, sigma=1.9)
tone = np.clip((tone - 0.018) / 0.38, 0, 1)
...
binary = tone_strong > 0.128
binary = binary_closing(binary, structure=np.ones((2, 2), dtype=bool))
binary = remove_small_objects(binary, min_size=22)
...
for cnt in contours:
    ...
    if local_dark < 0.070 and arc < 52:
        continue
    add_polyline_simplified("source_contour", pts, eps=1.15, min_len_px=12.0)


# Extra light contours only below the sky: useful detail in forest/field/figure, without noisy black cloud scribbles.
edge_img = np.clip(flat * 255.0, 0, 255).astype(np.uint8)
edges = cv2.Canny(edge_img, 54, 138, apertureSize=3, L2gradient=True)
...
for cnt in edge_contours:
    ...
    if ymid < 0.345:
        continue
    if arc < 48 or ww < 6 or hh < 3:
        continue
    local = float(np.mean(tone[max(0, y):min(h, y + hh), max(0, x):min(w, x + ww)]))
    if local < 0.040 and arc < 85:
        continue
    pts = [(float(p[0][0]), float(p[0][1])) for p in cnt]
    add_polyline_simplified("light_contour", pts, eps=1.55, min_len_px=24.0)
...
hatch_layer("sky_soft", sky_mask, 0.060, -25, spacing=22, step=6, min_run=38, jitter=0.85, keep=0.78, phase=3)
hatch_layer("sky_cross", sky_mask, 0.125, 28, spacing=30, step=6, min_run=34, jitter=0.65, keep=0.48, phase=9)
...
hatch_layer("field_slope", field_mask, 0.040, -13, spacing=10.5, step=6, min_run=54, jitter=0.75, keep=0.92, phase=4)
hatch_layer("field_cross", field_mask, 0.105, 17, spacing=16, step=6, min_run=40, jitter=0.65, keep=0.62, phase=7)
# Grass lower zone: controlled clumps, only where tone supports it.
hatch_layer("grass_base", grass_mask, 0.060, -82, spacing=11, step=5, min_run=17, jitter=0.85, keep=0.80, phase=2)
hatch_layer("grass_cross", grass_mask, 0.120, 76, spacing=14, step=5, min_run=15, jitter=0.75, keep=0.60, phase=6)
...
# 5b) Hand-placed soft sky hatch clusters: this is deliberate artistic structure, not pixel noise.
def add_sky_cluster(cx, cy, rx, ry, angle_deg, count, length_frac, cross=False):
    local_rng = random.Random(int(cx * 10000 + cy * 7000 + count * 13 + angle_deg * 5))
    theta = math.radians(angle_deg)
    e = np.array([math.cos(theta), math.sin(theta)], dtype=np.float32)
    p = np.array([-math.sin(theta), math.cos(theta)], dtype=np.float32)
    center = np.array([cx * w, cy * h], dtype=np.float32)
    for i in range(count):
        # sample inside ellipse, biased into loose bands instead of random static.
        band = (i / max(1, count - 1) - 0.5) * 2.0
        off_perp = band * ry * h * 0.72 + local_rng.uniform(-0.010, 0.010) * h
        off_along = local_rng.uniform(-0.52, 0.52) * rx * w
        mid = center + e * off_along + p * off_perp
        ell = (((mid[0] / w - cx) / rx) ** 2 + ((mid[1] / h - cy) / ry) ** 2)
        if ell > 1.05:
            continue
        seg_len = length_frac * w * local_rng.uniform(0.55, 1.05)
        wig = local_rng.uniform(-0.006, 0.006) * h
        pts = []
        for k in range(9):
            t = k / 8 - 0.5
            xy = mid + e * (t * seg_len) + p * (wig * math.sin((t + 0.5) * math.pi))
            ex = ((xy[0] / w - cx) / rx) ** 2 + ((xy[1] / h - cy) / ry) ** 2
            if 0 <= xy[0] < w and 0 <= xy[1] < h and ex <= 1.18:
                pts.append((float(xy[0]), float(xy[1])))
        add_polyline_simplified("sky_cluster_cross" if cross else "sky_cluster", pts, eps=0.8, min_len_px=13.0)

for args in [
    (0.125, 0.095, 0.160, 0.055, -35, 34, 0.070, False),
    (0.145, 0.235, 0.190, 0.052, -26, 22, 0.062, False),
    (0.305, 0.125, 0.135, 0.050, -32, 18, 0.052, False),
    (0.505, 0.285, 0.145, 0.050, -22, 16, 0.050, False),
    (0.770, 0.175, 0.170, 0.058, -27, 25, 0.060, False),
    (0.820, 0.285, 0.155, 0.052, -25, 18, 0.054, False),
    (0.180, 0.270, 0.165, 0.040, 30, 10, 0.045, True),
    (0.790, 0.300, 0.150, 0.040, 28, 9, 0.043, True),
]:
    add_sky_cluster(*args)
...
# Save G-code / NC.
name = "gemini_structured_cleanmax_sky_a4"
nc_path = OUT / f"{name}.nc"
gcode_path = OUT / f"{name}.gcode"
lines = [
    "; structured cleanmax sky A4 - coherent density hatching",
    "G21",
    "G90",
    f"G0 Z{PEN_UP_Z:.3f}",
    f"F{TRAVEL_F}",
...
readme.write_text(
    "STRUCTURED CLEANMAX SKY A4 package\n"
    f"source: {SRC}\n"
    f"nc: {nc_path}\n"
    f"gcode: {gcode_path}\n"
...
print("STRUCTURED CLEANMAX SKY A4 package")
