...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_tone_levels_a4_pack")
...
def write_gcode(paths, nc):
    lines=['; density tone levels A4','G21','G90',f'G0 Z{PEN_UP_Z:.3f}',f'F{TRAVEL_F}']
...
def main():
    ...
    add_layer(paths,dens,reg['sky'],angle=28,spacing=54,threshold=0.34,min_len=22,max_len=82,label='sky_light',jitter=0.22,bridge_px=2)
    add_layer(paths,dens,reg['sky'],angle=-24,spacing=64,threshold=0.48,min_len=20,max_len=68,label='sky_dark_cross',jitter=0.20,bridge_px=1)
    # Forest: density staircase, each darker level adds another direction.
    add_layer(paths,dens,reg['forest'],angle=-70,spacing=13,threshold=0.12,min_len=8,max_len=52,label='forest_l1',jitter=0.38,bridge_px=5)
    add_layer(paths,dens,reg['forest'],angle=-40,spacing=12,threshold=0.23,min_len=8,max_len=48,label='forest_l2',jitter=0.34,bridge_px=4)
    add_layer(paths,dens,reg['forest'],angle=45,spacing=10,threshold=0.36,min_len=7,max_len=38,label='forest_l3',jitter=0.30,bridge_px=3)
    ...
    add_layer(paths,dens,reg['field'],angle=12,spacing=20,threshold=0.13,min_len=28,max_len=150,label='field_l1',jitter=0.24,bridge_px=6)
    add_layer(paths,dens,reg['field'],angle=20,spacing=15,threshold=0.26,min_len=20,max_len=120,label='field_l2',jitter=0.22,bridge_px=4)
    ...
    add_layer(paths,dens,reg['grass'],angle=-72,spacing=15,threshold=0.18,min_len=5,max_len=25,label='grass_l1',jitter=0.48,bridge_px=1)
    add_layer(paths,dens,reg['grass'],angle=58,spacing=16,threshold=0.36,min_len=5,max_len=24,label='grass_l2',jitter=0.38,bridge_px=1)
    # Figure: close, readable crosshatch, no random detector noise.
    add_layer(paths,dens,reg['jacket'],angle=-54,spacing=5.8,threshold=0.12,min_len=18,max_len=96,label='jacket_l1',jitter=0.24,bridge_px=5)
    add_layer(paths,dens,reg['jacket'],angle=52,spacing=5.8,threshold=0.20,min_len=18,max_len=96,label='jacket_l2',jitter=0.24,bridge_px=4)
    add_layer(paths,dens,reg['jacket'],angle=12,spacing=6.5,threshold=0.40,min_len=16,max_len=82,label='jacket_l3',jitter=0.18,bridge_px=3)
    add_layer(paths,dens,reg['body'],angle=-38,spacing=9.0,threshold=0.22,min_len=16,max_len=82,label='body_l1',jitter=0.22,bridge_px=4)
    ...
    nc=OUT/'gemini_density_tone_levels_a4.nc'; gcode=OUT/'gemini_density_tone_levels_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_tone_levels_preview_pressure_gray.png'; bpng=OUT/'gemini_density_tone_levels_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_tone_levels_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_tone_levels_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY TONE LEVELS A4 package\n'
        ...
        'algorithm_note: tonal staircase. Light areas receive sparse long hatches; each darker threshold adds closer crossing layers. Hair uses curved strands, jacket uses clean close crosshatch, and only long contours are kept.\n'
        'files:\n- gemini_density_tone_levels_preview_pressure_gray.png/pdf\n- gemini_density_tone_levels_preview_black_actual.png/pdf\n- gemini_density_tone_levels_a4.nc\n- gemini_density_tone_levels_a4.gcode\n'
    )
