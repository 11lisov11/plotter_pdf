...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_organic_density_v2_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('field_light',field,.055,44,1,[-14,-9],(24,58),1,2.5,7,.90,21,90)
bundles('field_mid',field,.150,36,1,[-15,16],(18,42),2,2.4,7,.92,22,70)
bundles('grass',grass,.075,27,2,[-82,76,-68],(10,29),2,2.0,12,.91,31,220)
...
flow_curve('field_flow_light',0.535,14,slope=-0.12,amp=2.5,length_rng=(58,125),mask=field)
flow_curve('field_flow_mid',0.610,10,slope=-0.10,amp=2.8,length_rng=(48,105),mask=field)
flow_curve('field_flow_low',0.705,10,slope=-0.08,amp=3.2,length_rng=(40,90),mask=grass)
...
name='gemini_organic_density_v2_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; organic density v2 A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: organic capped density v2; field flow lines are reduced so light regions stay sparse while dark areas keep close parallel groups.\n',encoding='utf-8')
print('ORGANIC DENSITY V2 A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
