...
OUT=Path(r'C:\plotter_pdf\_plotter_jobs\gemini_directed_density_noicons_a4_pack'); OUT.mkdir(parents=True,exist_ok=True)
...
bundles('forest_dark',forest,.145,22,2,[-65,55,82],(10,24),2,2.1,12,.78,12,500)
bundles('forest_deep',forest,.270,19,2,[-68,52,84],(8,19),2,2.0,14,.75,13,350)
...
# forest texture accents: short diagonal strokes only, no symbolic tree icons
for i in range(90):
    cx=rng_flow.uniform(0.58*w,0.98*w); cy=rng_flow.uniform(0.345*h,0.505*h)
    ix=int(min(w-1,max(0,cx))); iy=int(min(h-1,max(0,cy)))
    if not forest[iy,ix]:
        continue
    a=math.radians(rng_flow.choice([-68,54,78])+rng_flow.uniform(-9,9)); ln=rng_flow.uniform(8,22)
    pts=[]
    for k in range(5):
        t=k/4-.5; bend=math.sin((t+.5)*math.pi)*rng_flow.uniform(-1.1,1.1)
        pts.append((cx+math.cos(a)*ln*t-math.sin(a)*bend, cy+math.sin(a)*ln*t+math.cos(a)*bend))
    clip('forest_texture',pts,forest,6)
# sky clusters
...
name='gemini_directed_density_noicons_a4'; nc=OUT/f'{name}.nc'; gcode=OUT/f'{name}.gcode'
lines=['; directed density noicons A4','G21','G90',f'G0 Z{UP:.3f}',f'F{TF}']; draw=travel=0; last=(0,0)
...
(OUT/'README_result.txt').write_text('LAYERED CAP A4 package\n'+f'source: {SRC}\n'+f'nc: {nc}\n'+f'gcode: {gcode}\n'+f'preview_black_actual_png: {black}\n'+f'preview_black_actual_pdf: {blackpdf}\n'+f'preview_pressure_gray_png: {grayp}\n'+f'preview_pressure_gray_pdf: {graypdf}\n'+f'paths_total: {len(ordered)}\n'+f'kind_counts: {dict(counts)}\n'+f'draw_length_m: {draw/1000:.2f}\n'+f'travel_length_m: {travel/1000:.2f}\n'+f'estimated_time_min_ideal: {(draw/(DF/60)+travel/(TF/60))/60:.1f}\n'+'algorithm_note: directed density without icon-like trees; forest uses abstract diagonal texture, field/grass sparse flow gestures, dark areas capped.\n',encoding='utf-8')
print('DIRECTED DENSITY NOICONS A4 package'); print('paths_total:',len(ordered)); print('kind_counts:',dict(counts)); print('draw_length_m:',round(draw/1000,2)); print('travel_length_m:',round(travel/1000,2)); print('estimated_time_min_ideal:',round((draw/(DF/60)+travel/(TF/60))/60,1)); print('preview:',black); print('pdf:',blackpdf); print('nc:',nc)
