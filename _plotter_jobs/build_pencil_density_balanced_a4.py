...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_pencil_density_balanced_a4_pack")
...
hatch("forest_light", forest_mask, 0.075, -63, spacing_px=15, step_px=5, min_run_px=28, jitter_px=0.9, keep=0.88, phase_px=2, open_px=1, close_px=6)
hatch("forest_mid", forest_mask, 0.145, 55, spacing_px=12, step_px=5, min_run_px=24, jitter_px=0.8, keep=0.84, phase_px=6, open_px=1, close_px=5)
...
hatch("field_light", field_mask, 0.040, -13, spacing_px=13, step_px=6, min_run_px=64, jitter_px=0.75, keep=0.82, phase_px=3, open_px=1, close_px=9)
hatch("field_mid", field_mask, 0.105, 17, spacing_px=18, step_px=6, min_run_px=46, jitter_px=0.65, keep=0.50, phase_px=7, open_px=1, close_px=7)

hatch("grass_light", grass_mask, 0.055, -80, spacing_px=14, step_px=5, min_run_px=18, jitter_px=0.75, keep=0.66, phase_px=2, open_px=1, close_px=4)
...
name = "gemini_pencil_density_balanced_a4"
nc_path = OUT / f"{name}.nc"
gcode_path = OUT / f"{name}.gcode"
lines = ["; pencil density balanced A4", "G21", "G90", f"G0 Z{PEN_UP_Z:.3f}", f"F{TRAVEL_F}"]
...
readme.write_text(
    "PENCIL DENSITY BALANCED A4 package\n"
    f"source: {SRC}\n"
    f"nc: {nc_path}\n"
    f"gcode: {gcode_path}\n"
...
print("PENCIL DENSITY BALANCED A4 package")
