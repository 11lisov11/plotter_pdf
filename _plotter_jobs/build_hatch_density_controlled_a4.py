...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_hatch_density_controlled_a4_pack")
OUT.mkdir(parents=True, exist_ok=True)
random.seed(6202201)
np.random.seed(6202201)
...
def add(kind: str, pts: list[tuple[float, float]], min_len=7.0):
    if len(pts) < 2:
        return
    clean = [pts[0]]
    length = 0.0
...
def simp(kind: str, pts: list[tuple[float, float]], eps=.95, min_len=7.0):
    if len(pts) < 2:
        return
...
def clip_run(kind: str, pts: list[tuple[float, float]], mask: np.ndarray, min_len=7.0):
...
def active(mask: np.ndarray, thr: float, close=5, open_=1, min_size=28):
    a = mask & (tone >= thr)
    if close:
        a = binary_closing(a, np.ones((close, close), bool))
    if open_:
        a = binary_opening(a, np.ones((open_, open_), bool))
    return remove_small_objects(a, min_size=min_size)

def split_and_add(kind: str, pts: list[tuple[float, float]], min_len: float, max_chunk: float | None, rng: random.Random):
    if not pts:
        return
    if not max_chunk:
        simp(kind, pts, .95, min_len)
        return
    chunk: list[tuple[float, float]] = []
    acc = 0.0
    quota = rng.uniform(max_chunk * .55, max_chunk * 1.10)
    last = None
    for p in pts:
        if last is not None:
...
def hatch(kind: str, mask: np.ndarray, thr: float, angle: float, spacing: float, step: float,
          min_run: float, jitter: float, keep: float, phase: float = 0.0, close=5,
          max_chunk: float | None = None, seed=0):
    a = active(mask, thr, close=close)
    rng = random.Random(seed + int(angle * 17) + int(thr * 1000) + int(spacing * 10))
    ...
    for i in range(-n//2, n//2 + 1):
        ...
        for u in us:
            ...
            if 0 <= x < w and 0 <= y < h and a[y, x]:
                wob = jitter * (0.65 * math.sin(.030*u + .53*i) + 0.35 * math.sin(.011*u + phase))
                pt = xy + p * wob
                run.append((float(pt[0]), float(pt[1])))
            else:
                if len(run) * step >= min_run:
                    split_and_add(kind, run, min_run*.70, max_chunk, rng)
                run = []
        if len(run) * step >= min_run:
            split_and_add(kind, run, min_run*.70, max_chunk, rng)

# True dark contours, filtered.
b = tone_dark > .165
...
# Region-specific density hatching.
# Sky stays sparse.
hatch("sky_light", sky, .075, -30, 32, 6, 34, .7, .34, 3, close=7, max_chunk=55, seed=10)
hatch("sky_cross", sky, .155, 30, 40, 6, 28, .55, .18, 9, close=6, max_chunk=38, seed=11)

# Forest: true dense zone, but controlled by line spacing.
hatch("forest_light", forest, .050, -62, 15.5, 5, 24, .75, .86, 2, close=6, max_chunk=44, seed=20)
hatch("forest_mid", forest, .125, 56, 12.5, 5, 20, .65, .86, 6, close=5, max_chunk=36, seed=21)
hatch("forest_dark", forest, .220, 86, 9.0, 5, 15, .45, .78, 1, close=4, max_chunk=28, seed=22)
hatch("forest_deep", forest, .300, -70, 7.8, 4, 12, .35, .58, 4, close=3, max_chunk=22, seed=23)

# Field: sparse, long slope strokes; no full technical grid.
hatch("field_light", field, .050, -13, 16.0, 6, 42, .50, .58, 4, close=8, max_chunk=95, seed=30)
hatch("field_mid", field, .150, 16, 22.0, 6, 32, .40, .32, 7, close=6, max_chunk=62, seed=31)

# Grass: fewer strokes than layered_cap, with directional texture.
hatch("grass_light", grass, .080, -80, 16.0, 5, 14, .55, .42, 2, close=4, max_chunk=30, seed=40)
hatch("grass_mid", grass, .165, 75, 14.5, 5, 12, .45, .34, 5, close=4, max_chunk=25, seed=41)

# Jacket/fabric: dense enough to read dark, but with larger spacing than black blob candidates.
hatch("jacket_light", jacket, .062, -52, 8.8, 4, 17, .36, .82, 0, close=5, max_chunk=42, seed=50)
hatch("jacket_mid", jacket, .135, 44, 8.5, 4, 16, .34, .80, 3, close=5, max_chunk=38, seed=51)
hatch("jacket_dark", jacket, .235, -74, 7.6, 4, 14, .28, .62, 1, close=4, max_chunk=32, seed=52)

hatch("hair_shadow", hair, .145, -72, 14.0, 4, 18, .25, .44, 2, close=5, max_chunk=42, seed=60)
...
def skycl(cx, cy, rx, ry, ang, cnt, lf, cross=False):
    ...
    for i in range(cnt):
        ...
        for k in range(8):
            t = k/7 - .5
            pts.append((x + e[0]*ln*t + p[0]*sr.uniform(-1.1,1.1), y + e[1]*ln*t + p[1]*sr.uniform(-1.1,1.1)))
...
name = "gemini_hatch_density_controlled_a4"
nc = OUT / f"{name}.nc"
gcode = OUT / f"{name}.gcode"
lines = ["; hatch density controlled A4", "G21", "G90", f"G0 Z{UP:.3f}", f"F{TF}"]
draw = travel = 0.0
last = (0.0, 0.0)
...
(OUT / "README_result.txt").write_text(
    "HATCH DENSITY CONTROLLED A4 package\n" +
    f"source: {SRC}\n" + f"nc: {nc}\n" + f"gcode: {gcode}\n" +
    ...
    "algorithm_note: controlled parallel hatch density; dark uses close multi-layer hatching, light remains sparse, semantic hair/fabric retained.\n",
    encoding="utf-8")
print("HATCH DENSITY CONTROLLED A4 package")
