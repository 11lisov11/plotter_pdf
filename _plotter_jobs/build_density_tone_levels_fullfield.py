...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_tone_levels_fullfield_a4_pack")
...
def write_gcode(paths, nc):
    lines=['; density tone levels full field A4','G21','G90',f'G0 Z{PEN_UP_Z:.3f}',f'F{TRAVEL_F}']
...
def main():
    ...
    add_layer(paths,dens,reg['field'],angle=12,spacing=15,threshold=0.08,min_len=28,max_len=165,label='field_l1',jitter=0.24,bridge_px=7)
    add_layer(paths,dens,reg['field'],angle=20,spacing=13,threshold=0.20,min_len=18,max_len=130,label='field_l2',jitter=0.22,bridge_px=5)
    ...
    nc=OUT/'gemini_density_tone_levels_fullfield_a4.nc'; gcode=OUT/'gemini_density_tone_levels_fullfield_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_tone_levels_fullfield_preview_pressure_gray.png'; bpng=OUT/'gemini_density_tone_levels_fullfield_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_tone_levels_fullfield_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_tone_levels_fullfield_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY TONE LEVELS FULL-FIELD A4 package\n'
        ...
        'algorithm_note: tonal staircase with restored sparse field/grass texture. Light areas receive sparse long hatches; each darker threshold adds closer crossing layers. Hair uses curved strands, jacket uses clean close crosshatch, and only long contours are kept.\n'
        'files:\n- gemini_density_tone_levels_fullfield_preview_pressure_gray.png/pdf\n- gemini_density_tone_levels_fullfield_preview_black_actual.png/pdf\n- gemini_density_tone_levels_fullfield_a4.nc\n- gemini_density_tone_levels_fullfield_a4.gcode\n'
    )
