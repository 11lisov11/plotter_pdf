...
OUT = Path(r"C:\plotter_pdf\_plotter_jobs\gemini_density_plus_cloudforest_a4_pack")
...
def write_gcode(paths, nc):
    lines=['; density plus cloud forest A4','G21','G90',f'G0 Z{PEN_UP_Z:.3f}',f'F{TRAVEL_F}']
...
def main():
    ...
    nc=OUT/'gemini_density_plus_cloudforest_a4.nc'; gcode=OUT/'gemini_density_plus_cloudforest_a4.gcode'
    draw,travel,line_count=write_gcode(ordered,nc); shutil.copy2(nc,gcode)
    ppng=OUT/'gemini_density_plus_cloudforest_preview_pressure_gray.png'; bpng=OUT/'gemini_density_plus_cloudforest_preview_black_actual.png'
    render(ordered,ppng,True); render(ordered,bpng,False)
    png_to_pdf(ppng,OUT/'gemini_density_plus_cloudforest_preview_pressure_gray.pdf')
    png_to_pdf(bpng,OUT/'gemini_density_plus_cloudforest_preview_black_actual.pdf')
    counts=defaultdict(int)
    for p in ordered: counts[p['kind']]+=1
    text=(
        'DENSITY PLUS CLOUDFOREST A4 package\n'
        ...
        'algorithm_note: plus_figurefolds with slightly richer cloud/forest/field texture; keeps figure density unchanged and avoids dark-area bundling. Light areas receive sparse long hatches; each darker threshold adds closer crossing layers. Hair uses curved strands, jacket uses clean close crosshatch, and only long contours are kept.\n'
        'files:\n- gemini_density_plus_cloudforest_preview_pressure_gray.png/pdf\n- gemini_density_plus_cloudforest_preview_black_actual.png/pdf\n- gemini_density_plus_cloudforest_a4.nc\n- gemini_density_plus_cloudforest_a4.gcode\n'
    )
