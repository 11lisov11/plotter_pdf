...
OUT_DIR = Path(r'C:\plotter_pdf\_plotter_jobs\gemini_grattage_scan_balanced_a4_pack')
NAME = 'gemini_grattage_scan_balanced_a4'
PAGE_W_MM = 180.0
PAGE_H_MM = 280.0
MARGIN_MM = 4.0
DRAW_FEED = 1400.0
...
scan_added = {}
scan_added['sky_cloud'] = add_scan_hatch('sky_cloud', .04, .34, .040, 20, [-26, 24], 4, 24, 150, .55)
scan_added['forest_cross'] = add_scan_hatch('forest_cross', .31, .53, .043, 7, [-62, 54], 3, 10, 1050, .82)
scan_added['forest_dark_extra'] = add_scan_hatch('forest_dark_extra', .32, .53, .092, 8, [-42], 3, 10, 260, .90)
scan_added['field_slope'] = add_scan_hatch('field_slope', .47, .70, .034, 11, [-14, -10], 4, 26, 360, .62)
scan_added['grass_front'] = add_scan_hatch('grass_front', .66, .99, .035, 10, [-82, 75], 3, 9, 650, .64)
scan_added['subject_shadow'] = add_scan_hatch('subject_shadow', .57, .99, .082, 7, [-55, 43], 3, 10, 650, .86)
...
def write_gcode(path: Path):
    draw = 0.0
    travel = 0.0
    lines = ['; grattage scan-hatch balanced A4', 'G21', 'G90', 'G0 Z3.500']
    cur = (0.0, 0.0)
    for p in ordered:
        sx, sy = p[0]
...
readme.write_text(
    f'GRATTAGE SCAN HATCH BALANCED A4\n'
    f'source: {SRC}\n'
...
print('GRATTAGE SCAN HATCH BALANCED A4 package')
