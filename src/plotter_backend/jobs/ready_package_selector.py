...
def _audit_items(variant_dir: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(items, list):
        return _summary_fallback_items(variant_dir)
    audit_items = [item for item in items if isinstance(item, dict)]
    return audit_items or _summary_fallback_items(variant_dir)


def _summary_fallback_items(variant_dir: Path) -> list[dict[str, Any]]:
    fallback_items: list[dict[str, Any]] = []
    for summary_path in sorted(variant_dir.glob("*_pack/summary.csv")):
        rows = [clean_report_value(row) for row in _csv_rows(summary_path)]
        ok_rows = [row for row in rows if str(row.get("ok") or "").strip().lower() == "true"]
        if not ok_rows:
            continue
        first = ok_rows[0]
        package_dir = summary_path.parent
        item_names = {str(row.get("item") or "").strip().lower() for row in ok_rows}
        fallback_kind = first.get("kind") or "drawing"
        if any(item_name.startswith("pass_") for item_name in item_names):
            fallback_kind = "a3_two_pass"
        fallback_items.append(
            {
                "task": package_dir.name,
                "kind": fallback_kind,
                "package_dir": str(package_dir),
                "layout_similarity": first.get("layout_similarity"),
                "selected_variant": first.get("selected_variant"),
            }
        )
...
def find_first_ready_package(variant_dir: Path, *, kind: str = "a4", item: str | None = None) -> ReadyPackageSelection:
    ...
    for item in _audit_items(variant_dir):
        ...
        item_kind = _normalize_kind(str(item.get("kind") or ""))
